...
Note, by default, the `levels` of a factor are the unique elements of the `character` vector in alphabetical order. You can specify an order by explicitly defining the levels as in:
...
Notice that the integer representation of the vector changed order, because the levels changed order.
