...
We saw above that we could use a logical vector in the bracket notation to extract the `TRUE` indices of a vector. In the case above we had a literal logical vector (i.e. `c(TRUE, FALSE, FALSE, FALSE)`). But we can also use a logical expression. Let's say we wanted get all of the countries in our vector of country populations that were less than 10,000,000. We could index using the '<' (less than) logical operator:
...
>
> The reason why the expression `populations[populations < 10000000]` works can be better understood if you examine what the expression `populations < 10000000` evaluates to:
>
> ```{r, purl = FALSE}
> populations < 10000000
> ```
>
...
> populations[populations < population_cutoff]
> ```
>
...
Before we briefly introduce matrices and lists, let's reset some vectors:
