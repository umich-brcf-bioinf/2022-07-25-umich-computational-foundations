...
As we saw with matrices, since data frames are two-dimensional, we have to specify both the row and column to extract specific entries, as in:
...
Now, let's read in the file `data/gapminder_data.csv` which is located in `/home/workshop/user/CF_R/data/gapminder_data.csv`. Call this data `gapminder`. The first argument to pass to our `read.csv()` function is the file path for our data. The file path must be in quotes and now is a good time to remember to use tab autocompletion. **If you use tab autocompletion you avoid typos and errors in file paths.** Use it!
...
A **data frame is the standard way in R to store tabular data**. Remember, a data frame is a collection of vectors, all of which have the same length. Using only two functions, we can learn a lot about out data frame including some summary statistics as well as well as the "structure" of the data frame. Let's examine what each of these functions can tell us:
...
If you recall, factors are ordered alphabetically by default. That might make sense in some cases, but some categories (e.g., "red", "blue", "green") often do not have an intrinsic order. What if we wanted to order our plot according to the numerical value (i.e., in ascending order of continent frequency)? We can enforce an order on our factors:

```{r, purl = FALSE}
# First let's look at the table of the continent column
table(gapminder$continent)

# Next, let's sort that table
sort(table(gapminder$continent))

# Noticing that the result is named, we can extract the names
names(sort(table(gapminder$continent)))

# And those can be used as the level order
reordered_continents <- factor(gapminder$continent, levels = names(sort(table(gapminder$continent))))

# And let's quickly look at the result using the head() function
head(reordered_continents, 20)
```

Now we see our plot has be reordered:

```{r, purl = FALSE}
plot(reordered_continents)
```

Factors come in handy in many places when using R. For example, in ggplot2 factors can help us reorder plots, and in statistical testing factors help us define the reference level.
...
# Bonus Material: Math and Sorting
